...
export default function ReportsPage() {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: "0 auto" }}>
      <h1 style={{ fontSize: "2rem", fontWeight: 600, marginBottom: 24 }}>Reports Dashboard</h1>
      <div style={{ display: 'flex', gap: 24, marginBottom: 32, flexWrap: 'wrap' }}>
        <DashboardCard title="Total Hours" value={`${totalHours}h ${totalMins}m`} color="#4f8cff" />
        <DashboardCard title="This Week" value={`${weekHours}h ${weekMins}m`} color="#00c48c" />
        <DashboardCard title="Top Member" value={topUsers[0]?.name || '-'} color="#ffb547" />
        <button onClick={exportCSV} style={{ background: '#0052cc', color: '#fff', border: 'none', borderRadius: 8, padding: '18px 32px', fontWeight: 600, fontSize: 18, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', marginLeft: 'auto' }}>Export CSV</button>
      </div>

      <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', marginBottom: 32 }}>
        <div style={{ flex: 2, minWidth: 340, background: '#fff', borderRadius: 12, boxShadow: '0 2px 16px rgba(0,0,0,0.07)', padding: 24 }}>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 12 }}>Activity Report (Last 7 days)</h2>
        ...
        <div style={{ flex: 1, minWidth: 320, background: '#fff', borderRadius: 12, boxShadow: '0 2px 16px rgba(0,0,0,0.07)', padding: 24 }}>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 12 }}>Top 5 Members based on time logged</h2>
          {loading ? (
            <div>Loading...</div>
          ) : (
      ...
      <div style={{ marginTop: 48, background: '#fff', borderRadius: 12, boxShadow: '0 2px 16px rgba(0,0,0,0.07)', padding: 32 }}>
        <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 18 }}>Top 5 Tasks by Time Spent</h2>
...
function TopTasksTable({ entries }: { entries: TimeEntry[] }) {
  ...
  const topTasks = Object.values(taskTotals)
    ...
    .sort((a, b) => b.totalMinutes - a.totalMinutes)
    .slice(0, 5);
  if (topTasks.length === 0) return <div style={{ color: '#888' }}>No task data available.</div>;
  return (
    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
      <thead>
        <tr style={{ background: '#f6f8fa' }}>
